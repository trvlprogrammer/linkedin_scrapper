...
from apps.linkedin_api.client import Client
from apps.linkedin_api.utils.helpers import (
    get_id_from_urn,
...
class Linkedin(object):
    ...
    def search(self, params, limit=-1, offset=0):
        ...
        print(divmod(time() - start_time,60))
    ...
    def get_profile_contact_info(self, public_id):
        ...
        try:
            ...
            if data.get("websites") != None:
                if len(data.get("websites")) >= 1 :
                    result["website"] = data.get("websites")[0].get("url") 

            if data.get("birthDateOn") != None:
                if data.get("birthDateOn").get("month") != None and data.get("birthDateOn").get("day") != None:
                    result["birthdate"] =  calendar.month_name[data.get("birthDateOn").get("month")] + " " + str(data.get("birthDateOn").get("day"))

            if data.get("phoneNumbers") != None:
                if len(data.get("phoneNumbers")) >=1 :
                    result["phone_numbers"] = data.get("phoneNumbers")[0].get("number")
            return result
    ...
    def get_profile(self, public_id):
        """
            ...
            return :
                dictionary
        """
        res = self._fetch(f"/identity/profiles/{public_id}/profileView")
        data = res.json()
        try:       
            result = {}
            profile = data.get("profile")            
            result["headline"] = profile.get("headline","")
            result["location"] = profile.get("geoLocationName","")
            result["country"] = profile.get("geoCountryName","")
            result["industry"] = profile.get("industryName","")
            ...
            result["summary"] = ""
            result["public_id"] = ""
            if profile.get("miniProfile") != None:
                if profile.get("miniProfile").get("occupation") != None :
                    company = profile.get("miniProfile").get("occupation").split("at")
                    if len(company) >=2 :
                        result["company"] = company[1]

                if "picture" in profile["miniProfile"]:
                    root_images = profile.get("miniProfile").get("picture").get(
                    ...
                    result["image"] = root_images + images_data
                
                if profile.get("miniProfile").get("publicIdentifier") != None:
                    result["public_id"] =profile.get("miniProfile").get("publicIdentifier")            

            if data.get("positionView") != None:
                if data.get("positionView").get("elements") != None:                        
                    experience = data.get("positionView").get("elements")
                    experience_data = []
                    for item in experience:
                        ...
                        if "company" in item and "miniCompany" in item["company"]:                            
                            if "logo" in item["company"]["miniCompany"]:
                                ...
                                if logo:
                                    company_logo = logo["rootUrl"]
                                if logo.get("artifacts") and len(logo.get("artifacts")) > 0:
                                    company_logo = company_logo + logo.get("artifacts")[0].get("fileIdentifyingUrlPathSegment") 
                        
                        start_date = ""
                        end_date = ""           
                                
                        if item.get("timePeriod") != None :
                            if item.get("timePeriod").get("startDate") != None:
                                start_date = calendar.month_name[item.get("timePeriod").get("startDate").get("month")] + " " + str(item.get("timePeriod").get("startDate").get("year"))
                            if item.get("timePeriod").get("endDate") != None:
                                end_date = calendar.month_name[item.get("timePeriod").get("endDate").get("month")] + " " + str(item.get("timePeriod").get("endDate").get("year"))                        

                        experience_data.append({"name":item["companyName"], "title" : item["title"],
                        "logo":company_logo,"url":"", "start_date" : start_date,"end_date" : end_date})
                    result["experience"] = experience_data

            if data.get("educationView")!= None:
                if data.get("educationView").get("elements") != None:
                    if len(data.get("educationView").get("elements")) > 0:
                        education = data.get("educationView").get("elements")[0].get("schoolName") 
                        result["education"] = education

            if profile.get("miniProfile") != None:
                if profile["miniProfile"].get("picture") != None:
                    root_images = profile.get("miniProfile").get("picture").get(
                        "com.linkedin.common.VectorImage"
                    ).get("rootUrl")
            ...
            if data.get("publicationView") != None:
                result["entity_id"] = data.get("publicationView").get("profileId")
            return result
        except Exception as e:
            return {}
    ...
    def get_location_urn(self,location):
        """
            Get location urn id
            params :
                Location Name
            result :
                list of urn location id
        """       
        uri = f"/voyagerSearchDashReusableTypeahead?decorationId=com.linkedin.voyager.dash.deco.search.typeahead.ReusableTypeaheadCollection-28&keywords={location}&q=type&query=(typeaheadUseCase:PROFILE)&type=GEO"  
        res = self._fetch(uri)
        if res.status_code == 200:
            res =  res.json()
            data = []
            for r in res["elements"]:
                data.append(r["trackingUrn"].replace("urn:li:geo:",""))
            return data
        else :
            return []
